    ...
    */
    ...
    TemplateLoaded(template) {
        ...
        this.LoadData().Done(dataSet => {
            ...
            if (this.Meta && this.Meta.Events.HasAnyChar()) {
                this.DispatchEvent(this.Meta.Events, "DOMContentLoaded", this, this.Entity).then();
            }
        });
    }
    ...
    LoadData() {
        ...
        let promise = new Promise((resolve, reject) => {
            let isFn = Utils.IsFunction(this.Meta.PreQuery, false, this);
            let sql = {
                ComId: this.Meta.Id,
                Params: JSON.stringify(isFn),
    ...
    ProcessData(dataSet) {
            ...
            for (let j = 0; j < dataSet[i].length; j++) {
                for (let prop in dataSet[i][j]) {
                    if (typeof dataSet[i][j][prop] === 'string') {
                        dataSet[i][j][prop] = dataSet[i][j][prop];
                    }
                }
            }
            if (i === 0) {
        ...
        Utils.IsFunction(this.Meta.FormatEntity, false, this);
        return res;
    }

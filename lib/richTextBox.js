        ...
        this.quill = (await tinymce.init({
            ...
            setup: function (editor) {
                                ...
                                if (file) {
                                    ...
                                    self.ImageHandler(
                                        ...
                                        function success(path) {
                                            var selectedNode = editor.dom.getParent(editor.selection.getStart(), 'table');
                                            editor.dom.setStyle(selectedNode, 'background-image', `url(${path})`);
                                            editor.dom.setStyle(selectedNode, 'background-size', 'cover');
                                            editor.dom.setStyle(selectedNode, 'background-repeat', 'no-repeat');
                                            editor.dom.setStyle(selectedNode, 'background-position', 'center');
                                            dialog.close();
                                        },
                ...
                editor.ui.registry.addMenuItem('gen-table-excel', {
                    ...
                    onAction: function () {
                        const a4WidthPx = 793.7; // Độ rộng A4 theo điểm ảnh (96dpi)
                        const a4HeightPx = 1000.5; // Chiều cao A4 theo điểm ảnh (96dpi)
                        const columnWidthPx = 80;
                        const rowHeightPx = 16.6;
                        ...
                        for (let r = 0; r < rows; r++) {
                            ...
                            for (let c = 0; c < columns; c++) {
                                tableHtml += '<td style="width:' + columnWidthPx + 'px; height:' + rowHeightPx + 'px;"></td>';
                            }
                            tableHtml += '</tr>';
                        }
                        tableHtml += '</table>';

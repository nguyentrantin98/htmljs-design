    ...
    */
    ...
    async DispatchClickAsync() {
        ...
        Html.Instance.End.Div.ClassName("icon-box d-flex").Style("display: flex; gap: 20px; align-items: center;").Span.ClassName("fa fa-times")
            .Event("click", handlerMail).End.End.End.Div.ClassName("popup-body scroll-content").Style("max-height:calc(100vh - 9rem) !important;display: flex; align-items: center;background-color:#525659");
        Html.Instance.Iframe.ClassName("container-rpt").Style("min-height: calc(-10rem + 100vh);margin:auto;background:#fff;overflow: auto;min-height:calc(100vh - 16rem);").Width("220mm");

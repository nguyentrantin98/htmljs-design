...
export class GridView extends ListView {
    ...
    ActionKeyHandler(e, header, focusedRow, com, el, keyCode) {
        ...
        if ([KeyCodeEnum.F4, KeyCodeEnum.F8, KeyCodeEnum.F9, KeyCodeEnum.F10, KeyCodeEnum.F11, KeyCodeEnum.F2, KeyCodeEnum.UpArrow, KeyCodeEnum.DownArrow, KeyCodeEnum.Home, KeyCodeEnum.End, KeyCodeEnum.Insert].includes(keyCode) || (e.ctrlKey || e.metaKey) && keyCode === KeyCodeEnum.D) {
            ...
            switch (com.Meta.ComponentType) {
                case "SearchEntry":
                    value = focusedRow.Entity[header.FieldName].toString().EncodeSpecialChar();
                    break;
                case "Number":
                    value = focusedRow.Entity[header.FieldName].toString().replace(",", "");
                    break;
                case "Checkbox":
    ...
    async RowChangeHandler(rowData, rowSection, observableArgs, component = null) {
        ...
        if (observableArgs.EvType === EventType.Change) {
            ...
            this.LastListViewItem = rowSection;
            if (["Dropdown", "Datepicker"].includes(this.LastComponentFocus.ComponentType) || addRow) {
                var headers = this.Header.filter(y => y.Editable);
                var currentComponent = headers.filter(y => y.FieldName == component?.Meta.FieldName)[0];
                if (currentComponent != null && rowData[currentComponent.FieldName] != null) {
                    var index = headers.indexOf(currentComponent);
                    if (headers.length > index + 1) {
                        var nextGrid = headers[index + 1];
                        var nextComponent = rowSection.Children.filter(y => y?.Meta.FieldName == nextGrid.FieldName)[0];
                        nextComponent.Focus();
                    }
                }
            }
    ...
    MoveGroupRow(rowSection) {
        ...
        if (groupSection) {
            ...
            var groupText = Utils.IsFunction(this.Meta.GroupFormat, false, rowSection.Entity);
            Html.Instance.TData.ClassName("status-cell").EndOf(ElementType.td)
                .TData.ColSpan(this.Header.length - 1)
                .Event(EventType.Click, () => this.DispatchClick(rowSection.Entity))
    ...
    HideColumn(...param) {
            ...
            this.HeaderSection.Children.forEach(column => {
                if (param.includes(column.Meta.FieldName)) {
                    if (column.Element.style.display == "") {
                        ...
                        if (parentElement) {
                            parentElement.Element.setAttribute("colspan", (parseInt(parentElement.Element.getAttribute("colspan")) - 1).toString())
                        }
                    }
        ...
        if (this.GridViewItemEmpty) {
            this.GridViewItemEmpty.Children.forEach(column => {
                if (param.includes(column.Meta.FieldName)) {
                    column.Element.parentElement.style.display = "none";
                }
                else {
                    column.Element.parentElement.style.display = "";
                }
            })
        }
        ...
        if (this.AllListViewItem) {
            this.AllListViewItem.forEach(row => {
                if (row.GroupRow) {
                    var newspan = this.Header.length - param.length;
                    row.Element.lastElementChild.setAttribute("colspan", newspan)
                }
                else {
        ...
        const columns = headerElements.map(header => {
            ...
            if (match && !header.Meta.StatusBar && Utils.isNullOrWhiteSpace(match.style.display)) {
                ...
                const dirtyPatch = [
                    ...
                    !anyGroup ? { Field: "Order", Value: header.Order } : { Field: "Order", Value: header.Meta.Order }
                ];

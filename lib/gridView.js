...
export class GridView extends ListView {
    ...
    async RowChangeHandler(rowData, rowSection, observableArgs, component = null) {
        var currency = rowSection.Children.find(x => x.IsSearchEntry && x.Meta.FieldName == "CurrencyId");
        if (currency) {
            rowData.ExchangeRateVND = EditableComponent.ExchangeRateVND[currency.GetValueText()] || 1;
            rowData.ExchangeRateUSD = EditableComponent.ExchangeRateUSD[currency.GetValueText()] || 1;
            rowData.CurrencyCode = currency.GetValueText();
        }

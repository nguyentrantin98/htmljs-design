    ...
    RenderTableHeader(headers) {
        ...
        Html.Take(this.HeaderSection.Element).Clear().TRow.ForEach(headers, (header, index) => {
            if (anyGroup && !Utils.isNullOrWhiteSpace(header.GroupName)) {
                ...
                Html.Th.Render();
                Html.ColSpan(headers.filter(x => x.GroupName === header.GroupName).length).IHtml(header.GroupName).End.Render();
                return;
            }
            Html.Th
        ...
        if (anyGroup) {
            Html.Instance.TRow.ForEach(headers, (header, index) => {
                if (anyGroup && !Utils.isNullOrWhiteSpace(header.GroupName)) {
                    ...
                    // @ts-ignore
                    const section = new Section(Html.Context.parentElement);
                    section.Meta = header;

...
export class GridView extends ListView {
    ...
    HideColumn(...param) {
        ...
        if (this.AllListViewItem) {
            this.AllListViewItem.forEach(row => {
                if (row.GroupRow) {
                    var newspan = this.Header.length + 1 - param.length;
    ...
    UpdateHeaders() {
        ...
        if (this.Token.SystemRole) {
            const columns = headerElements.map(header => {
                ...
                if (match && !header.Meta.StatusBar && Utils.isNullOrWhiteSpace(match.style.display)) {
                    ...
                    const dirtyPatch = [
                        ...
                        !Utils.isNullOrWhiteSpace(header.GroupName) ? { Field: "Width", Value: width } : { Field: "Order", Value: header.Meta.Width },
                        !Utils.isNullOrWhiteSpace(header.GroupName) ? { Field: "MaxWidth", Value: width } : { Field: "Order", Value: header.Meta.MaxWidth },
                        !Utils.isNullOrWhiteSpace(header.GroupName) ? { Field: "MinWidth", Value: width } : { Field: "Order", Value: header.Meta.MinWidth },
            ...
            const columns = headerElements.map(header => {
                const match = header.Element;
                if (match && !header.Meta.StatusBar && !Utils.isNullOrWhiteSpace(header.FieldName) && Utils.isNullOrWhiteSpace(match.style.display)) {
                    const width = `${match.offsetWidth}px`;
                    return {
                        Id: header.Id,
                        FieldName: header.FieldName,

...
export class GridView extends ListView {
    ...
    FilterColumns(components) {
        ...
        const permission = (!this.EditForm || this.EditForm.Policies.length == 0) ? [] : this.EditForm.GetGridPolicies(components.map(x => x.id), Utils.ComponentId);
        const headers = (permission.length == 0 ? components.filter(x => !x.hidden && x.id !== this.Meta.Id)
            .filter(header => !header.isPrivate || permission.filter(x => x.RecordId === header.Id)
                .every(policy => policy.CanRead)) : components)

...
export class AdvancedSearch extends EditableComponent {
    ...
    static OperatorFactory(componentType) {
        // @ts-ignore
        var entities = AdvSearchOperation.ToEntity();
        switch (componentType) {
            case ComponentType.Textbox:
                return entities.filter(x => (x.Id >= AdvSearchOperation.Contains && x.Id < AdvSearchOperation.In)
                    || x.Id == AdvSearchOperation.Equal || x.Id == AdvSearchOperation.NotEqual)
                    // @ts-ignore
                    .sort((x, y) => x.Id == AdvSearchOperation.Contains ? -1 : 1);
            case ComponentType.Datepicker:
                return entities.filter(x => x.Id < AdvSearchOperation.Contains);
            case ComponentType.Number:
                return entities.filter(x => x.Id < AdvSearchOperation.Contains);
            case ComponentType.Checkbox:
                return entities.filter(x => x.Id == AdvSearchOperation.Equal);
            case ComponentType.SearchEntry:
                return entities.filter(x => x.Id == AdvSearchOperation.In || x.Id == AdvSearchOperation.NotIn);
        }
        return null;
    }

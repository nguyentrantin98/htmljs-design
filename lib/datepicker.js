...
export class Datepicker extends EditableComponent {
    ...
    CloseCalendar() {
        this._closeAwaiter = setTimeout(() => {
            ...
            this.Input.value = this.value ? dayjs.tz(this.value, dayjs.tz.guess()).format(this.InitFormat) : "";
            this.hour = null;
            this.minute = null;
        }, 250);
    }
    ...
    RenderCalendarTask(someday) {
        ...
        if (this.calendar) {
            ...
            Html.Take(this.EditForm.Element || document.body).Div.ClassName("datepicker-content open open-up").TabIndex(-1).Trigger("focus");
        ...
        if (this.Meta.Precision === 7) {
            Html.Instance.Div.ClassName("time")
                ...
                .Event("change", this.ChangeHour.bind(this)).End.End.End.End.End
                ...
                .Event("change", this.ChangeMinute.bind(this)).EndOf(".time");
        }
    ...
    TryParseDateTime(value) {
        ...
        for (let i = 0; i < this.formats.length; i++) {
            dateTime = dayjs.tz(value, this.formats[i], dayjs.tz.guess()); // true for strict parsing
            if (dateTime.isValid()) {
                parsed = true;
    ...
    IncreaseTime(value, minute = false) {
        ...
        if (!minute) {
            ...
            this.hour.value = time.get('hour').toString().padStart(2, '0');
        } else {
            time.setMinutes(time.get('minute') + value);
            this.minute.value = time.get('minute').toString().padStart(2, '0');
        }
    ...
    ChangeHour(e) {
        ...
        this.TriggerUserChange(newDate);
    }

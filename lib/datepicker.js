...
export class Datepicker extends EditableComponent {
    ...
    CloseCalendar() {
        this._closeAwaiter = setTimeout(() => {
            this.calendar.removeEventListener("focusout", (e) => this.SetSelectedDay(e, this.Value));
            this.show = false;
            if (this.calendar) {
        ...
        if (this.Meta.Precision === 7) {
            Html.Instance.Div.ClassName("time")
                ...
                .Event("click", (e) => {
                    clearTimeout(this._closeAwaiter);
                    e.target.select();
                    this.calendar.addEventListener("focusout", (e) => this.SetSelectedDay(e, this.Value));
                })

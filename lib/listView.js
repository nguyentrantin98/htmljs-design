...
export class ListView extends EditableComponent {
    ...
    */
    ...
    async LoadHeader() {
        ...
        var pivotRow = columns.find(x => x.ComponentType == "Number" && x.IsMultiple);
        if (pivotRow) {
            ...
            headers.push({
                ...
                MinWidth: pivotRow.MinWidth,
                MaxWidth: pivotRow.MaxWidth,
                FormatData: `const vndAmount = this.Meta.ChildHeader.reduce((sum, header) => {
                                const fieldValue = this.Entity[header.FieldName] || 0;
                                return sum + Number(fieldValue);
                            }, 0).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                            return  vndAmount == '0' ? '' : vndAmount;`,
                GroupFormat: `
                    const vndAmount = this.ChildrenItems
                    .map(x => x.Entity)
                    .reduce((sum, entity) => {
                        return sum.plus(this.Cell.Meta.ChildHeader.reduce((subSum, header) => {
                            return subSum.plus(entity[header.FieldName] || 0);
                        }, new this.Decimal(0)));
                    }, new this.Decimal(0))
                    .toFixed(0)
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ',');

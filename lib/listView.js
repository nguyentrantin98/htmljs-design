...
export class ListView extends EditableComponent {
    ...
    */
    ...
    BodyContextMenuHandler(e) {
        ...
        if (!this.IsSearchEntry && this.Meta.IsMultiple) {
            this.SetSelected(e);
        }
    ...
    */
    ...
    copyRowWithoutId(selectedRows, path = null) {
        return selectedRows.map(row => {
            ...
            this.setPropValue(res, "NoSubmit", 0);
            this.setPropValue(res, "IsLock", 0);
    ...
    */
    ...
    RenderEditMenu() {
        ...
        var selected = this.GetSelectedRows();
        var check = selected.some(x => x["StatusId"] && [3, 4].includes(x["StatusId"]) || x["NoSubmit"] || x["IsLock"]);
        var some = selected.some(x => x["AssignId"] == this.Token.UserId || x["InsertedBy"] == this.Token.UserId);
        if ((this.Meta.CanDelete && !check) || some) {
    ...
    async ViewHistory(currentItem) {
        ...
        Html.Take(this.ParentElement ?? this.Parent?.Element ?? TabEditor.TabContainer).Div.ClassName("backdrop").TabIndex(-1).Trigger(EventType.Focus)
            .Style("align-items: baseline;").Escape((e) => this.DisposeViewHistory.bind(this));

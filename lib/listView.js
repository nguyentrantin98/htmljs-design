...
export class ListView extends EditableComponent {
    ...
    GetSql(skip = null, pageSize = null, cacheMeta = false, count = true) {
        ...
        const headers = this.Header.filter(x => !x.HasFilter && (x.ComponentType == "Input" || x.ComponentType == "Textarea" || x.ComponentType == "Dropdown"));
        const searchTerm = this.ListViewSearch.EntityVM.SearchTerm;
        const operatorsValue = headers.map(x => ComponentExt.MapToFilterOperatorValue(x, searchTerm)).filter(x => x != null);

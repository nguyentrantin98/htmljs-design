...
export class ListView extends EditableComponent {
    ...
    */
    ...
    GetSql(skip = null, pageSize = null, cacheMeta = false, count = true) {
        ...
        const allowedTypes = ["Dropdown", "Textarea", "Checkbox", "Input", "Datepicker"];
        const headers = this.Header.filter(x => allowedTypes.includes(x.ComponentType));
        ...
        var operatorsValue = headers.map(x => {
            ...
            if (mapCom && !Utils.isNullOrWhiteSpace(mapCom.GetValueText())) {
                if (mapCom instanceof Datepicker) {
                    ...
                    textFilter = ComponentExt.MapToFilterOperatorValue(x, mapCom.GetValue())
                }
                else {
        ...
        var orderby = this.SearchSection.Children.filter(x => x.IsOrderBy).map(x => "ds." + x.Meta.FieldName + " " + x.SearchMethod).Combine(x => x, ", ");
        /** @type {SqlViewModel} */
        // @ts-ignore
        var res = {
            ComId: this.Meta.Id,
            ...
            OrderBy: !orderby ? (!this.Meta.OrderBy ? "ds.InsertedDate desc" : this.Meta.OrderBy) : orderby,
            Where: finalCon,
            Count: count,
            Skip: skip,
            Top: pageSize,
    ...
    */
    ...
    async HardDeleteSelected() {
        ...
        if (deletedItems.length == 0 || check || this.Meta.IsMultiple) {
            return;
        }
        const confirmDialog = new ConfirmDialog();

...
export class ListView extends EditableComponent {
    ...
    async ReloadData(cacheHeader = false, skip = null, pageSize = null) {
        if (Utils.isNullOrWhiteSpace(this.Meta.RefName)) {
            var data = JSON.parse(this.Meta.Query);
    ...
    GetSql(skip = null, pageSize = null, cacheMeta = false, count = true) {
        ...
        const res = {
            ...
            OrderBy: orderBy || (!this.Meta.OrderBy ? "ds.InsertedDate desc" : this.Meta.OrderBy),
            Where: finalCon,
            Count: count,
            Skip: skip,
            Top: pageSize,
    ...
    SyncMasterData(rows = null, headers = null) {
        ...
        var locals = this.Header.filter(x => x.ComponentType == this.Dropdown && Utils.isNullOrWhiteSpace(x.RefName));
        for (const header of locals) {
            ...
            rows.forEach(row => {
                let data = JSON.parse(header.Query);
    ...
    FilterColumns(components) {
        ...
        let SpecificComponent = components.some(x => x.ComponentId === this.Meta.Id && !x.TopEmpty);
        if (SpecificComponent) {
            components = components.filter(x => x.ComponentId === this.Meta.Id && !x.TopEmpty);
        } else {
            components = components.filter(x => x.ComponentId === null && !x.TopEmpty);
        }
    ...
    */
    CopySelected(ev) {
        ...
        var dataCopy = {
            ...
            RowData: this._originRows
        }
        window["CopyData"] = dataCopy;
    ...
    */
    async PasteSelected(ev) {
        var dataCopy = window["CopyData"];
        if (!dataCopy || dataCopy.TableName != this.Meta.RefName) {
            return;
        }
        var copyRows = dataCopy.RowData;
        this._copiedRows = copyRows;
        if (this._copiedRows.length == 0) {
            return;
        }
        this._copiedRows.forEach(cell => {
            cell.Id = Uuid7.NewGuid();
        ...
        this.DispatchCustomEvent(this.Meta.Events, CustomEventType.BeforePasted, this._originRows, this._copiedRows).Done(() => {
            ...
            this.AddRowsNo(this._copiedRows, index).then(list => {
                super.Focus();
    ...
    RenderEditMenu(canWrite) {
        ...
        if (this.CanDo(x => x.CanDeactivate || x.CanDeactivateAll) && this.CanDelete) {
            // @ts-ignore
            ContextMenu.Instance.MenuItems.push({
                Icon: "fal fa-unlink",
                Text: "Deactivate",
        ...
        if (this.CanDo(x => x.CanDelete || x.CanDeleteAll) && this.CanDelete) {
            // @ts-ignore
            ContextMenu.Instance.MenuItems.push({
                Icon: "fal fa-trash",
                Text: "Delete Data",
    ...
    async ViewHistory(currentItem) {
        ...
        Html.Instance.Div.ClassName("popup-content confirm-dialog").Style("top: 0;")
            ...
            .EndOf(".popup-title")
            .Div.ClassName("card-body panel group").Width("700px");

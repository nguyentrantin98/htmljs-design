...
export class ListViewSearch extends EditableComponent {
    ...
    CalcFilterQuery() {
        ...
        var headers = this.Parent.Header.filter(x => x.ComponentType == "Dropdown" || x.ComponentType == "Textarea" || x.ComponentType == "Input");
        const searchTerm = this.EntityVM.SearchTerm;
        if (!Utils.isNullOrWhiteSpace(searchTerm)) {
            headers = headers.filter(x => x.ComponentType != "Datepicker");
        }
        var check = false;
        const operators = headers.map(x => {
            ...
            if (mapCom && mapCom.GetValueText()) {
                if (mapCom instanceof Datepicker) {
                    textFilter = ComponentExt.MapToFilterOperator(x, mapCom.Value.format('YYYY-MM-DD') || "", mapCom)
                }
                else {
                    textFilter = ComponentExt.MapToFilterOperator(x, mapCom.GetValueText() || "", mapCom)
                }
                check = true;
            }
            return textFilter;

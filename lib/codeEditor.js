...
export class CodeEditor extends EditableComponent {
    ...
    EditorLoaded() {
        ...
        if (this.editor.getModel()) {
            closeAllExceptFirst(this.editor);
        } else {
        ...
        function closeAllExceptFirst(editor) {
            editor.getAction('editor.foldAll').run().then(() => {
                const firstLine = 1;
                editor.setSelection({ startLineNumber: firstLine, startColumn: 1, endLineNumber: firstLine, endColumn: 1 });

...
export class CodeEditor extends EditableComponent {
    ...
    EditorLoaded() {
        ...
        function foldOnlyFunctions(editor) {
            const model = editor.getModel();
            if (!model) return;
        
            // Lấy nội dung của tất cả các dòng trong mô hình
            const lines = model.getLinesContent();
            const foldingRanges = [];
        
            // Biểu thức chính quy để phát hiện các khai báo hàm
            const functionRegex = new RegExp(
                [
                    /^[a-zA-Z0-9_]+\s*:\s*function\s*\(/.source, // Dạng object method
                    /^function\s+[a-zA-Z0-9_]+\s*\(/.source, // Dạng khai báo hàm truyền thống
                    /^[a-zA-Z0-9_]+\s*\(.*\)\s*=>/.source, // Dạng arrow function
                    /^async\s+function\s+[a-zA-Z0-9_]+\s*\(/.source, // Dạng async function
                    /^[a-zA-Z0-9_]+\s*:\s*async\s*function\s*\(/.source, // Async method trong object
                    /^async\s+[a-zA-Z0-9_]+\s*\(.*\)\s*=>/.source // Async arrow function
                ].join("|"),
                "i"
            );
        
            // Duyệt qua từng dòng và tìm các hàm
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i].trim();
        
                if (functionRegex.test(line)) {
                    // Thêm dòng hiện tại vào danh sách folding ranges
                    foldingRanges.push(i + 1); // Monaco Editor sử dụng chỉ mục bắt đầu từ 1
                }
            }
        
            // Chỉ đóng nếu có hơn 5 hàm
            if (foldingRanges.length > 5) {
                foldingRanges.forEach(lineNumber => {
                    editor.setSelection({ startLineNumber: lineNumber, startColumn: 1, endLineNumber: lineNumber, endColumn: 1 });

...
export class Select extends EditableComponent {
    ...
    RenderInputAndEvents() {
        if (this.Element == null) {
            this.Element = this._input = Html.Take(this.ParentElement).Div.Position(PositionEnum.relative).TabIndex(-1).ClassName(this.SEntryClass).Div.TabIndex(-1).ClassName("input-select").GetContext();

...
export class SearchEntry extends EditableComponent {
    ...
    constructor(ui, ele = null) {
        ...
        this._gv = null;
        let containId = this.Meta.FieldName.substr(this.Meta.FieldName.length - 2) === this.IdField;
        if (containId) {
            this.DisplayField = this.Meta.FieldName.substr(0, this.Meta.FieldName.length - 2);
        }
        else {
    ...
    RenderIcons() {
        ...
        let div = Html.Instance.Icon('fa fa-info-circle').Title(LangSelect.Get('Thông tin chi tiết ') + LangSelect.Get(this.Meta.Label))
            .Event('click', this.OpenRefDetail.bind(this)).End
            .Icon('fa fa-plus').Title(`${title} ${LangSelect.Get(this.Meta.Label)}`).Event('click', this.OpenRefDetail.bind(this)).End.GetContext();
    ...
    FindMatchText() {
        if (Utils.isNullOrWhiteSpace(this.Meta.RefName)) {
            ...
            this.Matched = this.Entity[this.DisplayField] || data.filter(x => x.Id == this.Entity[this.Meta.FieldName])[0];
    ...
    GetMatchedText(matched) {
        ...
        let objField = "";
        if (containId) {
            objField = this.Meta.FieldName.substr(0, this.Meta.FieldName.length - 2);
        }
        else {
            objField = this.Meta.FieldName + "MasterData";
        }
        if (matched === null && this.Entity === null || !matched) {
            this.Entity[objField] = null;
            return '';
        }
        this.Entity[objField] = matched;
    ...
    EntrySelected(rowData) {
        ...
        if (this.Entity !== null && this.Name) {
            ...
            this.Entity[this.DisplayField] = rowData;
        }
        this.Dirty = true;
        this.Matched = rowData;
        this.SetMatchedValue();

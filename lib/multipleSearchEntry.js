...
export class MultipleSearchEntry extends SearchEntry {
    ...
    SetEntityValue() {
        ...
        this.Entity[this.Name + "Text"] = this.MatchedItems.length > 0 ? this.MatchedItems.map(item => this.GetMatchedText(item)).join(this.Meta.GroupFormat || ',') : null;
    }
    ...
    ProcessLocalMatch() {
        if (Utils.isNullOrWhiteSpace(this.Meta.RefName)) {
            ...
            this.MatchedItems = data.filter(x => x.Id == this._listValues.includes(this.Entity[this.Meta.FieldName]));
            this.SetMatchedValue();
            return true;
        }
        else {
    ...
    RenderTag(item) {
        ...
        Html.Take(this.Element.parentElement).Span.Attr("data-id", idAttr).InnerHTML(this.GetMatchedText(item));

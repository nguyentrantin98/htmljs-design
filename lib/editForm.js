...
export class EditForm extends EditableComponent {
            ...
            if (allItem && allItem.length > 0) {
                allItem.forEach(it => {
                    ...
                    if (currency) {
                        ...
                        it.Entity.CurrencyCode = currency.GetValueText();
                        it.Entity.ExSaleUSD = (EditableComponent.ExchangeRateSaleUSD[currency.GetValueText()] || it.Entity.ExProfitUSD);
                        it.Entity.ExSaleVND = (EditableComponent.ExchangeRateSaleVND[currency.GetValueText()] || it.Entity.ExchangeRateVND);
                        it.Entity.ExProfitUSD = (EditableComponent.ExchangeRateProfitUSD[currency.GetValueText()] || it.Entity.ExProfitUSD);
                        it.Entity.ExProfitVND = (EditableComponent.ExchangeRateProfitVND[currency.GetValueText()] || it.Entity.ExchangeRateVND);
                    }
            ...
            if (allItem && allItem.length > 0) {
                allItem.forEach(it => {
                    ...
                    if (currency) {
                        ...
                        it.Entity.ExSaleUSD = (EditableComponent.ExchangeRateSaleUSD[currency.GetValueText()] || it.Entity.ExchangeRateUSD);
                        it.Entity.ExSaleVND = (EditableComponent.ExchangeRateSaleVND[currency.GetValueText()] || it.Entity.ExchangeRateVND);
                        it.Entity.ExProfitUSD = (EditableComponent.ExchangeRateProfitUSD[currency.GetValueText()] || it.Entity.ExchangeRateUSD);
                        it.Entity.ExProfitVND = (EditableComponent.ExchangeRateProfitVND[currency.GetValueText()] || it.Entity.ExchangeRateVND);
                    }
                    if (multiples && multiples.length > 0) {
                        multiples.forEach(item1 => {
    ...
    async SavePatch(reloadData) {
        ...
        try {
            ...
            var currency = this.ChildCom.find(x => x.IsSearchEntry && x.Meta.FieldName == "CurrencyId" && Utils.isNullOrWhiteSpace(x.Meta.EntityName));
            if (currency) {
                ...
                this.Entity.CurrencyCode = currency.GetValueText();

                this.Entity.ExSaleUSD = (EditableComponent.ExchangeRateSaleUSD[currency.GetValueText()] || this.Entity.ExchangeRateUSD);
                this.Entity.ExSaleVND = (EditableComponent.ExchangeRateSaleVND[currency.GetValueText()] || this.Entity.ExchangeRateVND);
                this.Entity.ExProfitUSD = (EditableComponent.ExchangeRateProfitUSD[currency.GetValueText()] || this.Entity.ExchangeRateUSD);
                this.Entity.ExProfitVND = (EditableComponent.ExchangeRateProfitVND[currency.GetValueText()] || this.Entity.ExchangeRateVND);
            }
            if (multiples && multiples.length > 0) {
    ...
    async SaveEntity(entityForm, tableName) {
        ...
        try {
            ...
            var currency = this.ChildCom.find(x => x.IsSearchEntry && x.Meta.FieldName == "CurrencyId" && x.Meta.EntityName == entityForm);
            if (currency) {
                currentEntity.ExchangeRateVND = EditableComponent.ExchangeRateVND[currency.GetValueText()] || 1;
                currentEntity.ExchangeRateUSD = EditableComponent.ExchangeRateUSD[currency.GetValueText()] || 1;
                currentEntity.CurrencyCode = currency.GetValueText();

                currentEntity.ExSaleUSD = (EditableComponent.ExchangeRateSaleUSD[currency.GetValueText()] || this.Entity.ExchangeRateUSD);
                currentEntity.ExSaleVND = (EditableComponent.ExchangeRateSaleVND[currency.GetValueText()] || this.Entity.ExchangeRateVND);
                currentEntity.ExProfitUSD = (EditableComponent.ExchangeRateProfitUSD[currency.GetValueText()] || this.Entity.ExchangeRateUSD);
                currentEntity.ExProfitVND = (EditableComponent.ExchangeRateProfitVND[currency.GetValueText()] || this.Entity.ExchangeRateVND);
            }
            if (multiples && multiples.length > 0) {
                multiples.forEach(item => {
    ...
    UpdateData(form) {
        ...
        if (childEntity && !form[childEntity.Meta.EntityName].Id.startsWith("-")) {
    ...
    async IsFormValid(showMessage = true) {
        ...
        const validationPromises2 = this.ChildCom.filter(x => x.IsListView).flatMap(x => x.Item).flatMap(x => x.Children).filter(x => x.Meta.Validation).map(x => {
            return { IsValid: x.ValidateAsync(), com: x }
        });
    ...
    GetComPolicies(components) {
        ...
        var policyFeature = this.Policies.sort((a, b) => b.CanRead - a.CanRead).find(x => !x.RecordId && (Client.Token.RoleIds.some(k => k == x.RoleId) || Client.Token.UserId == x.UserId))
    ...
    async OpenPopup(featureName, entity, loadEntity, entitys) {
        if (!entity) {
            entity = {
                Id: Uuid7.NewGuid()
            }
    ...
    UpdateView2(force = false, dirty = null, entityName = null, updateTable = false, ...componentNames) {
        if (componentNames && componentNames.length > 0) {
    ...
    OpenConfig(title, yesConfirmed, noConfirmed, needAnswer, com, ignoreNoButton, componentGroup) {
        if (!this.Entity) {
            this.Entity = {};
        }
        const confirmDialog = new ConfirmDialog();

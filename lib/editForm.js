...
export class EditForm extends EditableComponent {
                ...
                allItem.forEach(it => {
                    ...
                    if (multiples.length > 0) {
                        multiples.forEach(item1 => {
                            it.Entity[item1.Meta.FieldName + "Text"] = item1.MatchedItems.length > 0 ? item1.MatchedItems.map(item2 => item1.GetMatchedText(item2)).join(item1.Meta.GroupFormat || ',') : null;
                        })
                    }
                })
                var rowData = allItem.flatMap(x => x.Entity);
                /**
                 * @type {object[]}
                 */
                var itemEntity = rowData.map((row) => {
        ...
        try {
            ...
            if (multiples.length > 0) {
                multiples.forEach(item => {
                    this.Entity[item.Meta.FieldName + "Text"] = item.MatchedItems.length > 0 ? item.MatchedItems.map(item1 => item.GetMatchedText(item1)).join(item.Meta.GroupFormat || ',') : null;
                })
            }
            Spinner.AppendTo(this.Element);
            const valid = await this.IsFormValid();
     ...
     * @param {string} title
     */
    async OpenConfirmDialog(yesConfirmed, noConfirmed, title, needAnswer, com) {
        const confirmDialog = new ConfirmDialog();
    ...
    OpenConfig(title, yesConfirmed, noConfirmed, needAnswer, com) {
        const confirmDialog = new ConfirmDialog();
        confirmDialog.Title = title;
        confirmDialog.NeedAnswer = needAnswer;
        confirmDialog.Component = com;

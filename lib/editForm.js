...
export class EditForm extends EditableComponent {
    ...
    async SavePatch() {
        ...
        try {
            ...
            if (this.Entity[this.IdField] == null || !this.Entity[this.IdField]) {
                this.Entity[this.IdField] = Uuid7.NewGuid();
            }
            let dirtyPatch = [];
            ...
            var patchModels = gridItem.map(item => {
                ...
                var itemEntity = rowData.map((de) => {
                    ...
                    Object.getOwnPropertyNames(de).forEach(x => {
                        if (de[x] instanceof Array || (de[x] instanceof Object && !(de[x] instanceof Decimal))) {
                            return;
                        }
   ...
   */
    ...
    async LoadFeatureAndRender(callback = null) {
        ...
        if (this.Meta.Id) {
            this.LoadEntity().then(entity => {
                this.LayoutLoaded(this.Meta, callback);
            });
        }
        else {
            const featureTask = ComponentExt.LoadFeature(this.Meta.FieldName);
            this.LoadEntity().then((entity) => {
                featureTask.then(async feature => {
                    if (feature.ParentId) {
                        const featureParent = await Client.Instance.GetByIdAsync("Feature", [feature.ParentId]);
                        ComponentExt.AssignMethods(featureParent.data[0], this);
                    }
                    if (feature.Script) {
                        ComponentExt.AssignMethods(feature, this);
                    }
                    this.LayoutLoaded(feature, callback);
                });
            });
        }
    ...
    static SplitChild(hTMLElements, section = "", editForm = null) {
        for (const eleChild of hTMLElements) {
            ...
            if (eleChild.dataset.name !== undefined) {
                ...
                const component = ComponentFactory.GetComponent(ui, editForm.EditForm, eleChild);
                if (component == null) return;
                // @ts-ignore
                if (component.IsListView) {
                    // @ts-ignore
                    editForm.EditForm.ListViews.push(component);
                }
            ...
            setTimeout(() => {
                ...
                printWindow.addEventListener("afterprint", async (e) => {
                    if (component && component.Events && component.Events.AfterPrint) {
                        await this.DispatchEvent(component.Events.AfterPrint, "afterprint", this);
                    }
                    printWindow.close();
                });
    ...
    async SectionProperties(group) {
        ...
        const editor = new instanse.SectionEditor();
    ...
    async AddProperties() {
        ...
        const editor = new instanse.FeatureEditor();
    ...
    async ComponentProperties(component) {
        ...
        const editor = new md.ComponentEditor();
    ...
    async OpenPopup(featureName, entity) {
        import('./popupEditor.js').then((instanse) => {
            ...
            popup.Entity = entity;
            popup.ParentElement = this.Element;
            popup.OpenFrom = this;

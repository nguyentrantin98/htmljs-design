...
export class EditForm extends EditableComponent {
    ...
    GetPatchVM() {
        ...
        Object.getOwnPropertyNames(this.Entity).forEach(cell => {
            if (this.Entity[cell] instanceof Array || (this.Entity[cell] instanceof Object && !(this.Entity[cell] instanceof Decimal))) {
                return;
            }
            let val;
        ...
        try {
            ...
            var patchModels = gridItem.map(item => {
                ...
                if (item.AllListViewItem && item.AllListViewItem.length > 0) {
                    var rowData = item.AllListViewItem.filter(x => !x.GroupRow).flatMap(x => x.Entity);
                    /**
                     * @type {object[]}
                     */
                    var itemEntity = rowData.map((row) => {
            ...
            if (rs.status == 200) {
                ...
                for (const grid of gridItem) {
                    ...
                    for (const item of grid.AllListViewItem) {
                        var entity = dataItem.find(x => item.EntityId.includes(x.Id));
                        item.Entity = entity;
                ...
                var parent = this.OpenFrom.FilterChildren(x => x.IsTabComponent);
                for (const element of parent) {
                    await element.CountBadge();
                    if (element.FilterChildren(x => x.IsListView)[0]) {
                        var listViewItem = element.FilterChildren(x => x.IsListView)[0].AllListViewItem.find(x => x.Entity.Id == this.Entity.Id);
    ...
    RenderTabOrSection(componentGroup, editForm) {
        ...
        componentGroup = this.GetComPolicies(Utils.ComponentId, componentGroup);
     ...
     */
    GetComPolicies(entityName = "Component", components) {
        ...
        var policyFeature = this.Policies.find(x => x.TableName == "Feature" && (Client.Token.RoleIds.includes(x.RoleId) || Client.Token.UserId == x.UserId))
        var newComponents = components.map(com => {
            var check2 = this.Policies.find(x => x.TableName == entityName && x.RecordId == com.Id && (Client.Token.RoleIds.includes(x.RoleId) || Client.Token.UserId == x.UserId));
    ...
    async ActSendEntity() {
        ...
        var changes = this.GetPatchVM();
        var patchModel = new SavePatchVM();
        patchModel.Changes = changes;
        patchModel.Table = this.Meta.EntityId;
    ...
    async ActApprovedEntity() {
        ...
        if (res.status == 200) {
            ...
            Toast.Warning("There was an error with your approved.")
        }
    }

    async ActDeclineEntity() {
        ...
        if (res.status == 200) {
            ...
            Toast.Warning("There was an error with your decline.")
        }
    }
}
...
export class EditForm extends EditableComponent {
    ...
    GetComPolicies(components) {
        ...
        var policyFeature = this.Policies.sort((a, b) => b.CanRead - a.CanRead).find(x => !x.RecordId && (Client.Token.RoleIds.some(x => x == x.RoleId) || Client.Token.UserId == x.UserId))
        var newComponents = components.map(com => {
            var check2 = this.Policies.sort((a, b) => b.CanRead - a.CanRead).find(x => x.RecordId && x.RecordId == com.Id && (Client.Token.RoleIds.some(x == x.RoleId) || Client.Token.UserId == x.UserId));
    ...
    UpdateView2(force = false, dirty = null, entityName = null, updateTable = false, ...componentNames) {
        ...
        if (!updateTable) {
            var com = this.ChildCom.find(x => x.Meta.EntityName == entityName);
            if (com) {
                var comListView = this.ChildCom.find(x => x.Meta.RefName == com.Meta.TableName && x.IsListView);
                if (comListView) {

...
export class EditForm extends EditableComponent {
    ...
    GetComPolicies(components) {
        ...
        var policyFeature = this.Policies.sort((a, b) => b.CanRead - a.CanRead).find(x => !x.RecordId && (Client.Token.RoleIds.some(x.RoleId) || Client.Token.UserId == x.UserId))
        var newComponents = components.map(com => {
            var check2 = this.Policies.sort((a, b) => b.CanRead - a.CanRead).find(x => x.RecordId && x.RecordId == com.Id && (Client.Token.RoleIds.some(x.RoleId) || Client.Token.UserId == x.UserId));

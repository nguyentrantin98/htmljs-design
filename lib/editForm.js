...
export class EditForm extends EditableComponent {
    ...
    async SavePatch(reloadData) {
        ...
        try {
            ...
            if (rs.status == 200) {
                ...
                if (parent.length == 0) {
                    var gridDetail = this.ChildCom.find(x => x.IsListView);
                    if (reloadData) {
                    ...
                    for (const element of parent) {
                        ...
                        var gridDetail = element.FilterChildren(x => x.IsListView).find(x => x.IsListView);
                        if (gridDetail) {
                            if (reloadData) {
                                gridDetail.ReloadData();
                            }
    ...
    GetComPolicies(components) {
        ...
        var policyFeature = this.Policies.find(x => !x.RecordId && (Client.Token.RoleIds.includes(x.RoleId) || Client.Token.UserId == x.UserId))
        var newComponents = components.map(com => {
            var check2 = this.Policies.find(x => x.RecordId && x.RecordId == com.Id && (Client.Token.RoleIds.includes(x.RoleId) || Client.Token.UserId == x.UserId));
    ...
    async OpenPopup(featureName, entity) {
        ...
        import('./popupEditor.js').then((instanse) => {
            ...
            popup.Entity = entity;
            popup.ParentElement = this.TabEditor.Element;
            popup.OpenFrom = this;
    ...
    OpenConfig(title, predicate) {
        const confirm = new ConfirmDialog();
        confirm.Title = title;
        confirm.PElement = this.Element;
        confirm.YesConfirmed.add(predicate);
        confirm.Render();
    }
    ...
    DeclineEntity() {
        ...
        confirm.YesConfirmed.add(async () => {
            ...
            await this.ActDeclineEntity(confirm.Entity.ReasonOfChange);
        });
        confirm.Render();
    }
    ...
    async ActSendEntity() {
        ...
        if (res.status == 200) {
            ...
            if (parent.length > 0) {
                for (const element of parent) {
                    await element.CountBadge();
                    var gridDetail = element.FilterChildren(x => x.IsListView)[0];
                    if (gridDetail) {
                        gridDetail.ReloadData();
                    }
                }
            } else {
                var gridDetail = this.OpenFrom.FilterChildren(x => x.IsListView && x.Meta.RefName == this.Meta.EntityId)[0];
                if (gridDetail) {
                    gridDetail.ReloadData();
                }
            }
    ...
    async ActApprovedEntity() {
        ...
        if (res.status == 200) {
            ...
            if (parent.length > 0) {
                for (const element of parent) {
                    await element.CountBadge();
                    var gridDetail = element.FilterChildren(x => x.IsListView && x.Meta.RefName == this.Meta.EntityId)[0];
                    if (gridDetail) {
                        gridDetail.ReloadData();
                    }
                }

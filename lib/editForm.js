...
export class EditForm extends EditableComponent {
    ...
    async SavePatch(reloadData) {
        if (this.OpenFrom.IsTab) {
            this.Entity.Url = `${Client.BaseUri}/#/${this.Token.TenantCode || "app"}/${this.OpenFrom.FeatureName}?Popup=${this.FeatureName}&Id=${(this.Entity.Id.startsWith("-") ? "" : "")}`;
        }
        if (!this.Dirty) {
    ...
    Focus() {
        if (!this.Popup) {
            ...
            if (this.FeatureName) {
                this.Href = `${Client.BaseUri}/#/${this.Token.TenantCode || "app"}/${this.FeatureName}`;
            ...
            if (this.OpenFrom.IsTab) {
                this.Href = `${Client.BaseUri}/#/${this.Token.TenantCode || "app"}/${this.OpenFrom.FeatureName}?Popup=${this.FeatureName}&Id=${this.EntityId}`;
                window.history.pushState(null, LangSelect.Get(this.TabTitle), this.Href);
            }
            else if (this.OpenFrom.OpenFrom.IsTab) {
                this.Href = `${Client.BaseUri}/#/${this.Token.TenantCode || "app"}/${this.OpenFrom.OpenFrom.FeatureName}?Popup=${this.OpenFrom.FeatureName}&Id=${this.OpenFrom.EntityId}&Popup2=${this.FeatureName}&Id2=${this.EntityId}`;
                window.history.pushState(null, LangSelect.Get(this.TabTitle), this.Href);
            }
        }
        this.Show = true;
    ...
    Dispose() {
        if (this.OpenFrom && this.OpenFrom.IsTab) {
            window.history.pushState(null, LangSelect.Get(this.OpenFrom.TabTitle), `${Client.BaseUri}/#/${this.Token.TenantCode || "app"}/${this.OpenFrom.FeatureName}`);
        }
        super.Dispose();

...
export class EditForm extends EditableComponent {
    ...
    GetEntityPatchVM(entityForm, tableName) {
        ...
        let dirtyPatch = [];
        Object.getOwnPropertyNames(entityForm).forEach(cell => {
            if (entityForm[cell] instanceof Array || (entityForm[cell] instanceof Object && !(entityForm[cell] instanceof Decimal)) || cell == this._groupKey) {
                return;
            }
            let val;
            if (typeof entityForm[cell] === "boolean") {
                val = entityForm[cell] ? "1" : "0";
            } else {
                val = entityForm[cell];
            }
            let patchDetail = new PatchDetail();

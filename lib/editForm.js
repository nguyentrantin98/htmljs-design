...
export class EditForm extends EditableComponent {
    ...
    GetEntityPatchVM(entityForm, tableName) {
        ...
        var patchModels = gridItem.map(item => {
            ...
            var allItem = item.AllListViewItem.filter(x => !x.GroupRow && !x.Entity.NoSubmit);
    ...
    async SaveEntity(entityForm, tableName) {
        ...
        try {
            ...
            if (rs.status == 200) {
                ...
                if (comListView) {
                    var rowItem = comListView.AllListViewItem.find(x => x.Entity.Id = currentEntity.Id)
                    if (rowItem && rowItem.Entity && rowItem.Entity.Id && rowItem.Entity.Id && !addRow) {
                        rowItem.Entity = currentEntity;
                        rowItem.UpdateView(true);
                    }
                    else {
                ...
                if (rs.updatedItem[0]) {
                    var comListView = this.ChildCom.find(x => x.Meta.RefName == tableName && x.IsListView);
                    if (comListView) {
                        var rowItem = comListView.AllListViewItem.find(x => x.Entity.Id = rs.updatedItem[0].Id)
                        if (rowItem && rowItem.Entity && rowItem.Entity.Id && !addRow) {
                            rowItem.Entity = rs.updatedItem[0];
                            rowItem.UpdateView(true);
                        }
                        else {
                            comListView.ReloadData();

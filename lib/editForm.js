...
export class EditForm extends EditableComponent {
    ...
    GetPatchVM(selected) {
        var gridItem = this.ChildCom.filter(x => x.IsListView && x.Meta.Editable && !x.Meta.IsRealtime && Utils.isNullOrWhiteSpace(x.Meta.EntityName)
            && x.AllListViewItem.some(y => !y.GroupRow && ((selected && y.Selected) || (!selected))));
        let dirtyPatch = [];
        ...
        var patchModels = gridItem.map(item => {
            ...
            var allItem = item.AllListViewItem.filter(x => !x.GroupRow && ((selected && x.Selected) || (!selected)));
    ...
    async SavePatch(reloadData, selected) {
        ...
        try {
            ...
            await this.DispatchCustomEvent(this.Meta.Events, "onsave", this, gridItem);
            var patchModel = this.GetPatchVM(selected);
    ...
    async SaveEntity(entityForm, tableName) {
        ...
        try {
            ...
            const rs = await Client.Instance.PatchAsync(patchModel);
            if (rs.status == 200) {
                var codeEditor = this.ChildCom.filter(x => (x.Meta.ComponentType == "CodeEditor" || x.Meta.ComponentType == "Word") && !Utils.isNullOrWhiteSpace(x.Meta.RefName) && x.Meta.EntityName == entityForm);
                codeEditor.forEach(async item => {
                    if (rs.updatedItem[0][item.Meta.FieldName] == item.OldValue) {
                        return;
                    }
                ...
                currentEntity = rs.updatedItem[0];
                this[entityForm] = currentEntity;
                this.Dirty = false;
                this.UpdateView2(true);
                for (const grid of gridItem) {
                    grid.DeleteTempIds = [];
                    if (!grid.AllListViewItem || grid.AllListViewItem.length == 0) {
                        continue;
                ...
                Toast.Warning(rs.message || "Update detail fail");
            }
            return false;
        } catch (error) {
            Toast.Warning(error.message);
    ...
    Dispose() {
        ...
        if (this.Parent && this.Parent.Popup) {
            ...
            if (!this.Parent.Dirty && this.Parent.EntityId) {
                var childEntity = this.Parent.ChildCom.find(x => !Utils.isNullOrWhiteSpace(x.Meta.EntityName) && !Utils.isNullOrWhiteSpace(x.Meta.TableName));
                if (childEntity && !this.Parent[childEntity.Meta.EntityName].Id.startsWith("-")) {
                    Client.Instance.GetByIdAsync(childEntity.Meta.TableName, [this.Parent[childEntity.Meta.EntityName].Id]).then(entity => {
                        if (entity.data && entity.data[0]) {
                            this.Parent[childEntity.Meta.EntityName] = entity.data[0];
                            this.Parent.UpdateView2(false, false);
                        }
                    });
                }
                Client.Instance.GetByIdAsync(this.Parent.Meta.EntityId, [this.Parent.EntityId]).then(entity => {
                    if (entity.data && entity.data[0]) {
                        this.Parent.Entity = entity.data[0];
                        this.Parent.UpdateView(false, false);
                    }
                });
            }
        }
    ...
    async OpenPopup(featureName, entity, loadEntity) {
        if (!entity) {
            entity = {
                Id: Uuid7.NewGuid()
            }
    ...
    UpdateView(force = false, dirty = null, ...componentNames) {
        ...
        this.ChildCom.filter(x => Utils.isNullOrWhiteSpace(x.Meta.EntityName) && x.IsListView).forEach(/**@param {GridView} child **/ child => {
            child.Entity = child.Meta.EntityName ? this[child.Meta.EntityName] : this.Entity;
            child.PrepareUpdateView(force, dirty);
            if ((!com && !child.Meta.Editable) || (com && child.Meta.RefName != com.Meta.TableName && !child.Meta.Editable)) {
                child.ReloadData();
            }
        });
    }

    UpdateView2(force = false, dirty = null, ...componentNames) {
        if (componentNames && componentNames.length > 0) {
            this.ChildCom.filter(x => !Utils.isNullOrWhiteSpace(x.Meta.EntityName) && componentNames.includes(x.Meta.FieldName)).forEach(child => {
                child.Entity = child.Meta.EntityName ? this[child.Meta.EntityName] : this.Entity;
                child.UpdateView(force, dirty);
            });
            return;
        ...
        var com = this.ChildCom.find(x => !Utils.isNullOrWhiteSpace(x.Meta.EntityName) && x.IsListView);
        this.ChildCom.filter(x => x.IsListView && x.Meta.RefName == com.Meta.TableName).forEach(/**@param {GridView} grid **/ grid => {
            grid.ReloadData();
        });
    }

    ReloadUpdateView2(force = false, dirty = null, ...componentNames) {
        if (componentNames && componentNames.length > 0) {
            this.ChildCom.filter(x => !Utils.isNullOrWhiteSpace(x.Meta.EntityName) && componentNames.includes(x.Meta.FieldName)).forEach(child => {
                child.Entity = child.Meta.EntityName ? this[child.Meta.EntityName] : this.Entity;
                child.UpdateView(force, dirty);

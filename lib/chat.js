    ...
    */
    ...
    HandleMessage(data) {
        ...
        if (this.Token.UserId == item.FromId) {
            Html.Instance.Div.ClassName("message text-only").Div.ClassName("response").P.ClassName("text").Text(item.Message).End.End.End.Render();
        }
        else {
            if (this.LastFromId != item.FromId) {
                ...
                Html.Instance.Div.ClassName("message")
                    ...
                    .Div.ClassName("online").End.End.P.ClassName("text").Text(item.Message).End.End.Render();
            }
            else {
                Html.Instance.Div.ClassName("message text-only").P.ClassName("text").Text(item.Message).End.End.Render();
            }
        }
        if (this.Token.UserId == item.FromId) {
    ...
    RenderDiscussions() {
        ...
        this.UserParentElement = Html.Context;
        Html.Instance.End.Div.Render();
        this.BodyDiscussions = Html.Context;
        this.RenderBodyDiscussions();
        Html.Instance.EndOf(".discussions");
        var userIds = this.EditForm.Meta.Components && this.EditForm.Meta.Components.length ? this.EditForm.Meta.Components.find(x => x.FieldName == "UserIds") : this.TabEditor.Meta.Components.find(x => x.FieldName == "UserIds");
        userIds.CanRead = true;
        userIds.CanReadAll = true;
        userIds.CanWrite = true;
        userIds.CanWriteAll = true;
        var userDropdown = new MultipleSearchEntry(userIds);
        userDropdown.ParentElement = this.UserParentElement;
        this.AddChild(userDropdown);
        userDropdown.Element.parentElement.style.width = "100%";
    }
    LastFromId;
    LastUserId;
    /**
    ...
    RenderBodyDiscussions() {
        ...
        this.Conversation.forEach(item => {
            Html.Instance.Div.TabIndex(-1).Event(EventType.Click, (evt) => this.HandlerClick(evt, item)).ClassName("discussion " + ((item.Id == this.Entity.Id) ? "message-active" : ""))
                ...
                .P.ClassName("name").Text(item.FormatChat).End
                .P.ClassName("message").Text(item.Message).End.End

    ...
    */
    ...
    HandleMessage(data) {
        ...
        if (this.Token.UserId == item.FromId) {
            Html.Instance.Div.ClassName("message text-only").Div.ClassName("response").P.ClassName("text2").Text(item.Message).End.End.End.Render();
        }
        else {
    ...
    AttachFile(event) {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                this.HtmlIputChat.value += ` [File: ${file.name}](${fileUrl}) `;
            };

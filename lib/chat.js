...
export class Chat extends EditableComponent {
    ...
    */
    ...
    HandleMessage(data) {
        ...
        if (this.Token.UserId == item.FromId) {
            Html.Instance.Div.ClassName("message text-only").Div.ClassName("response")
                .Div.ClassName("text2").InnerHTML(item.Message).End
                .I.ClassName("icon fa fa-trash clickable").Event(EventType.Click, () => this.DeleteMessage(item)).End
                .End.End.Render();
        }
        else {
            if (this.LastFromId != item.FromId) {
                ...
                Html.Instance.Div.ClassName("message text-only").P.ClassName("text").InnerHTML(item.Message).End.End.Render();
            }
        }
        if (this.Token.UserId == item.FromId) {
    ...
    RenderBodyChat() {
        ...
        this.ChatData.forEach(item => {
            if (this.Token.UserId == item.FromId) {
                Html.Instance.Div.ClassName("message text-only").Div.ClassName("response")
                    .P.ClassName("text2").InnerHTML(item.Message).End
                    .I.ClassName("icon fa fa-trash clickable").Event(EventType.Click, () => this.DeleteMessage(item)).End
                    .End.End.Render();
            }
            else {
    ...
    */
    ...
    DeleteMessage(item) {
        ...
        // For example, you can send a request to the server to delete the message
        // and then remove it from the ChatData array and re-render the chat
        this.ChatData = this.ChatData.filter(msg => msg.Id !== item.Id);
        this.RenderBodyChat();
    }

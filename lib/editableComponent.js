...
import { Component, Action, EventType, ValidationRule } from "./models/";
...
export class EditableComponent {
    ...
    /** @type {Token} */
    get Token() {
        return Token.parse(Client.Token.AccessToken)
    }
    /** @type {TabEditor} Handle toggle event. */
    get TabEditor() {
    ...
    SetDefaultVal() {
        ...
        if (data && this.Entity[this.Name] == null) {
    ...
    PopulateFields(entity = null) {
        ...
        var fn = new Function(this.Meta.PopulateField);
        fn.call(root, entity);
    }
    ...
    UpdateView(force = false, dirty = null, ...componentNames) {
        ...
        if (componentNames.length > 0) {
            ...
            // @ts-ignore
            const shouldUpdate = this.FilterChildren(x => x.ComponentType != "Section" && x.ComponentType != "GridView");
            shouldUpdate.forEach(child => {
                child.PrepareUpdateView(force, dirty);
                child.Entity = this.Entity;
                child.UpdateView(force, dirty, ...componentNames);
    ...
    AddChild(child, index = null, showExp = null, disabledExp = null) {
        ...
        if (disabledExp || (child.Meta && child.Meta.DisabledExp)) {

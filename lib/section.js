...
export class Section extends EditableComponent {
    ...
    static RenderGroupContent(parent, groupInfo, width, entity, editForm) {
        ...
        if (groupInfo.IsDropDown) {
            Html.Instance.Details.Summary.IText(groupInfo.Label).End.Render();
        }
        else {
        ...
        if (groupInfo.Label && !groupInfo.IsDropDown && !groupInfo.IsTab) {
            Html.Instance.Label.ClassName("header").IText(groupInfo.Label).End.Render();
        }
    ...
    RenderComponent(group) {
        ...
        var lastElementButtonGroup = null;
        ...
        group.Components.sort((a, b) => a.Order - b.Order).forEach((ui, index) => {
            ...
            Html.Div.ClassName("layout-item").Style(`grid-column: span ${colSpan}`).Visibility(ui.Visibility);
            ...
            // Handle button group
            if (!Utils.isNullOrWhiteSpace(ui.GroupFormat) && ui.ComponentType === "Button") {
                if (!lastButtonGroup) {
                    lastButtonGroup = ui;
                    Html.Instance.Div.ClassName("dropdown-btn")
                        .Button.ClassName("dropbtn")
                        .IText(ui.GroupFormat)
                        .End
                        .Div.ClassName("dropdown-content dropdown-top");
                    lastElementButtonGroup = Html.Context;
                }
            }

            const childCom = ComponentFactory.GetComponent(ui, this.EditForm);
            if (!Utils.isNullOrWhiteSpace(ui.GroupFormat) && ui.ComponentType === "Button") {
                childCom.ParentElement = lastElementButtonGroup;
            }
            ...
            if (childCom.Element) {
                ...
                if (Client.SystemRole && ui.ComponentType !== "CodeEditor") {
            ...
            if (column === colgroup) {
                column = 0;
                Html.EndOf(".ui-row");
                if (index + 1 != group.Components.length) {
                    Html.Instance.Div.ClassName("ui-row").Style(`grid-template-columns: repeat(${colgroup}, 1fr);`).Render();
                }
            }
        });
    }
    ...
    RenderComponent2(group) {
        ...
        group.Components.sort((a, b) => a.Order - b.Order).forEach(ui => {
            ...
            if (!Utils.isNullOrWhiteSpace(ui.GroupFormat) && ui.ComponentType == "Button") {
                if (!lastButtonGroup) {
                    lastButtonGroup = ui;
                    Html.Instance.Div.ClassName("dropdown-btn").Button.ClassName("dropbtn").IText(ui.GroupFormat).End
                        .Div.ClassName("dropdown-content dropdown-top");
                    lastElementButtonGroup = Html.Context;
                }
            }

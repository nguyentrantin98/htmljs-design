...
export class ComponentExt {
    ...
    static MapToFilterOperator(component, searchTerm) {
        ...
        if (component.ComponentType === "Dropdown" && !Utils.isNullOrWhiteSpace(component.RefName)) {
            var sqlmap = this.ExtractStrings(component.FormatData).map(x => {
                return `charindex(${searchParam}, ds2.[${x}]) >= 1`
            });
            return `exists (select ds2.Id from [${component.RefName}] ds2 where ds2.Id = ${fieldName} and (${sqlmap.join(" and ")}))`;
        }
        else if (component.ComponentType === "Datepicker") {
            return `${fieldName} >= ${searchParam} AND ${fieldName} < DATEADD(day, 1, ${searchParam})`;
        }
        return `charindex(${searchParam}, ${fieldName}) >= 1`;
    }

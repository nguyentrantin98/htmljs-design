...
export class Utils {
    ...
    */
        ...
        switch (header.ComponentType) {
            case 'Datepicker':
                ...
                if (!cellData) {
                    ...
                    text = dayjs(cellData).utc().format(this.isNullOrWhiteSpace(header.FormatData) ? 'DD/MM/YYYY' : header.FormatData)
                }
                break;
            case 'Dropdown':
            case 'Select2':
            ...
            default:
                ...
                if (!this.isNullOrWhiteSpace(header.FormatEntity)) {
                    text = this.IsFunction(header.FormatEntity, false, editForm, row);
                    if (!fn) {
                        text = this.GetFormattedRow(header.FormatEntity, row);
                    }
                }
    ...
    static IsFunction(exp, shouldAddReturn = false, params) {
        ...
        shouldAddReturn = exp && !exp.includes("return");
        if (!exp || exp == null || exp == "") {
            return null;
        }
        try {

...
export class Image extends EditableComponent {
    ...
    */
    ...
    openPopupIFrame(url, img) {
        ...
        const handlerDowload = this.DowloadPdf.bind(this, url);
        Html.Take(this.TabEditor?.Element ?? document.body).Div.ClassName("backdrop").Style("align-items: center;").Escape(handlerClose);
        this.Preview = Html.Context;
        Html.Instance.Div.ClassName("popup-content");
        this.PopupContent = Html.Context;
        Html.Instance.Div.ClassName("popup-title").Span.IText("View FILE");
        ...
        Html.Instance.End.Div.ClassName("icon-box d-flex").Style("display: flex; gap: 20px; align-items: center;")
            .Span.ClassName("far fa-download").Event("click", handlerDowload).End
        ...
        Html.Instance.Iframe.ClassName("container-rpt").Style("margin:auto;background:#fff;overflow: auto;min-height:calc(100vh - 10rem);").Width("100%");
        this.IFrameElement = Html.Context;
        if (img) {
            var imgSrc = document.createElement('img');
            imgSrc.style = "max-width: 100%;margin: auto;display:flex";
            imgSrc.src = url;
            this.IFrameElement.contentDocument.body.appendChild(imgSrc);
        }
        else {

...
const LangComponent = () => {
    ...
    const calcLang = (data, cul) => {
        const map = data.filter(x => x.LangCode == cul).reduce((acc, cur) => {
    ...
    */
    ...
    const handleClick = (event, cul) => {
        ...
        Travel(document.documentElement).forEach(item => {
            ...
            props.split(",").forEach(propName => {
                ...
                const translated = dictionaryItems[template] !== undefined ? dictionaryItems[template] : template;
    ...
    const dropdownContent = (
        <>
            <a onClick={(e) => handleClick(e, "vi")} className="dropdown-item" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <img
                    width={"31px"}
            ...
            <a onClick={(e) => handleClick(e, "en")} className="dropdown-item" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <img
                    width={"31px"}
